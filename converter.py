def group_converter(group, face_value):
    ...
    if(len(group)==3):
        if int(group)!=0:#prevent returning face value (thousands name) if it is 000 eg:-1000000=1 million not 1 millon thousand
            if(group[1]!='1'):#special case not for ?10 to ?19
                if group[1]!='0' or group[2]!='0':#for triplets with tens and units place defined
                    if face_value==0:#for first triplet
                        word = hundreds_namer(group[0])+' and' +tens_namer(group[1]) +units_namer(group[2])+unit[face_value]
